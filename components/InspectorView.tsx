                    ...
                    {status === InspectionStatus.FAIL && result && (
                      <div className="space-y-8 pt-8 border-t border-slate-100 animate-in fade-in slide-in-from-top-4 duration-300">
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-3 bg-slate-50/50 border border-slate-200 rounded-4xl p-6 shadow-inner">
                            {(result.descriptionItems || []).map((item) => (
                              <div key={item.id} className="flex items-start gap-3">
                                <input
                                  type="checkbox"
                                  checked={item.completed}
                                  onChange={(e) => {
                                    const updated = (result.descriptionItems || []).map(i =>
                                      i.id === item.id ? { ...i, completed: e.target.checked } : i
                                    );
                                    updateItemDetails(selectedCategory.id, result.id, { descriptionItems: updated });
                                  }}
                                  className="w-5 h-5 cursor-pointer accent-brand-500 rounded mt-1 flex-shrink-0"
                                />
                                <span className={`font-bold leading-relaxed flex-1 ${item.completed ? 'line-through text-slate-400' : 'text-slate-700'
                                  }`}>
                                  {item.text}
                                </span>
                                <button
                                  onClick={() => {
                                    const updated = (result.descriptionItems || []).filter(i => i.id !== item.id);
                                    updateItemDetails(selectedCategory.id, result.id, { descriptionItems: updated });
                                  }}
                            ...
                            <input
                              ...
                              onKeyPress={(e) => {
                                if (e.key === 'Enter' && e.currentTarget.value.trim()) {
                                  ...
                                  const updated = [...(result.descriptionItems || []), newItem];
                                  updateItemDetails(selectedCategory.id, result.id, { descriptionItems: updated });
                                  e.currentTarget.value = '';
                                }
                              }}
                              className="w-full px-4 py-3 bg-white border border-slate-200 rounded-2xl text-base outline-none focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 transition-all font-bold"
                            />
                          </div>
                        </div>
...
const StatusButton: React.FC<{ label: string, active: boolean, color: string, onClick: () => void }> = ({ label, active, color, onClick }) => (
  <button
    onClick={onClick}
    className={`btn-modern px-6 py-4 rounded-2xl flex items-center justify-center font-black text-xs tracking-[0.1em] transition-all duration-500 shadow-sm ${active ? `${color} text-white shadow-xl shadow-${color.split('-')[1]}-200/50 ring-4 ring-white` : 'bg-white text-slate-400 hover:text-slate-600 border border-slate-200/50'
      }`}
  >
    {label}
  </button>
